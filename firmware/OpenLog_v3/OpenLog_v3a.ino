...
byte append_file(char* file_name)
{
  ...
  while(escape_chars_received < setting_max_escape_character) {
    if (TIFR0 & (_BV(OCF0A))) {
      ...
      a6 = analogRead(A6);
      a7 = analogRead(A7);
      sprintf_P((char*)localBuffer, PSTR("%3x %3x %3x %3x %3x %3x\n"), a0, a1, a4, a5, a6, a7);
      //NewSerial.println((char*)localBuffer);
      n = strlen((char*)localBuffer);
      workingFile.write(localBuffer, n); //Record the buffer to the card
      
      synccount++;
      if(synccount > 640 || (millis() - lastSyncTime) > MAX_TIME_BEFORE_SYNC_MSEC) //This will force a sync approximately every 5 seconds
      { 
        //This is here to make sure a log is recorded in the instance
        //where the user is throwing non-stop data at the unit from power on to forever
        workingFile.sync(); //Sync the card
...
void command_shell(void)
{
  ...
  while(true)
  {
    ...
    else if(strcmp_P(command_arg, PSTR("adc")) == 0)
    {
      int sensorValue;
      sensorValue = analogRead(A0);
      NewSerial.print(F("a0 "));
      NewSerial.print(sensorValue, HEX); //Print in HEX
      NewSerial.println();
      sensorValue = analogRead(A1);
      NewSerial.print(F("a1 "));
      NewSerial.print(sensorValue, HEX); //Print in HEX
      NewSerial.println();
      sensorValue = analogRead(A4);
      NewSerial.print(F("a4 "));
      NewSerial.print(sensorValue, HEX); //Print in HEX
      NewSerial.println();
      sensorValue = analogRead(A5);
      NewSerial.print(F("a5 "));
      NewSerial.print(sensorValue, HEX); //Print in HEX
      NewSerial.println();
      sensorValue = analogRead(A6);
      NewSerial.print(F("a6 "));
      NewSerial.print(sensorValue, HEX); //Print in HEX
      NewSerial.println();
      sensorValue = analogRead(A7);
      NewSerial.print(F("a7 "));
      NewSerial.print(sensorValue, HEX); //Print in HEX
      NewSerial.println();
    }
    /*
